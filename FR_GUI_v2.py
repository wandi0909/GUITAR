...
if selected_index == 0:
    ...
    def TypeDestroy():
        ...
        #read in buttons for vtr
            ...
            upld = Button(ws, text='Load Files', command = lambda:uploadFiles_vtr(file_path, c3))
            upld.place(in_ = addatbtn, y = 0, relx = spacing)
            ...
            for j in range(L):
                ...
                if Plotstr == 'Tw':
                    if contr == 'X': 
                        ...
                        if boxon3 % 2 == 0:
                            plot1.set_title("Twistmap no. "+str(j), fontsize = titlesize)
                        else: 
    ...
    def uploadFiles_npz_only(file_path_npz, c):
        ...
        for j in range(0, len(file_path_npz)):
            ...
            twist1.append(np.transpose(Test[int(j)]['twist']))
        
        global dim, MinX, MaxX, MinY, MaxY, MinZ, MaxZ, dx, dy, dz, contr
            
            #keylist = list(Test.keys())
        if ((len(Test[0]['x']) > 1) & (len(Test[0]['y']) > 1)):
            global zz
            zz = Test[0]['z']
            #print(np.shape(Test[0]['z']))
            dim = [len(Test[0]['y']), len(Test[0]['x'])]
            MinX = min(Test[0]['x'])
            MaxX = max(Test[0]['x'])
            MX = (MaxX-MinX)
            dx = MX/(dim[1]-1)
    ...
    def uploadFiles_npz(file_path_npz, c):
        Test = []
        global twist1
        twist1 = []
        for j in range(0, len(file_path_npz)):
    ...
    def uploadFiles(file_path):
        ...
        upld2 = Button(ws, text='Load Files', command = lambda:uploadFiles_npz(file_path_npz, c2))
        upld2.place(in_ = add_btn, y = 0, relx = spacing)
    
    def uploadFiles_vtr(file_path, c):
        Dataset = pv.read(file_path)
        ...
        for j in range(0, len(file_path)):
            twist1[j,:,:] = np.reshape(Dataset[j]['twist'], [dim[0],dim[1]])
    ...
    def Vis(contr, twist, num, num2, c, cc):
        ...
        if contr == 'X': 
            ...
            plot1.set_title("Twistmap no. "+str(num), fontsize = 9)
    ...
    def saveanim_tw(arr, filename, contr):
        ...
        labelsize, vmin, vmax, cmap_to_use):
            ...
            if contr == 'X':
                ...
                if boxon3 % 2 == 0:
                    ax_anim.set_title("Twistmap", fontsize = titlesize)
                else: 
